...
private:
	...
	using destroy_guard = detail::destroy_guard<T>;
...
public:
	...
	void assign(size_type count, const value_type& value)
	{
		...
		else
		{
			...
			memory_guard mguard(new_begin, new_cap);
	...
	iterator insert(const_iterator pos, size_type count, const value_type& value)
	{
		...
		if (count + old_size <= capacity())
		{
			...
			if (count > elems_after)
			{
				...
				destroy_guard dguard1(m_end, m_end + (count - elems_after));

				std::uninitialized_move_n(insert_pos, elems_after, insert_pos + count);
				destroy_guard dguard2(insert_pos + count, insert_pos + count + elems_after);
			...
			{
				std::uninitialized_move_n(m_end - count, count, m_end);
				destroy_guard dguard(m_end, m_end + count);

				std::move_backward(insert_pos, m_end - count, m_end);
		...
		else
		{
			...
			destroy_guard dguard1(insert_pos, insert_pos + count);

			std::uninitialized_move_n(m_begin, offset, new_begin);
			destroy_guard dguard2(new_begin, insert_pos);

			std::uninitialized_move_n(m_begin + offset, old_size - offset, insert_pos + count);
		...
		if constexpr (std::forward_iterator<InputIt>)
		{
			...
			if (count + old_size <= capacity())
			{
				...
				if (count > elems_after)
				{
					...
					std::uninitialized_move(insert_pos, m_end, m_end + count - elems_after);
					destroy_guard dguard2(insert_pos + count, insert_pos + count + elems_after);
			...
			else
			{
				...
				std::uninitialized_move(m_begin, m_begin + offset, new_begin);
				destroy_guard dguard2(new_begin, insert_pos);

				std::uninitialized_move(m_begin + offset, m_end, insert_pos + count);
	...
	iterator emplace(const_iterator pos, Args&&... args)
	{
		...
		if (m_end != m_end_cap)
		{
			...
			else
			{
				...
				++m_end;
				destroy_guard dguard(m_end - 1, m_end);
		...
		else
		{
			...
			destroy_guard dguard1(insert_pos, insert_pos + 1);

			std::uninitialized_move_n(m_begin, offset, new_begin);
			destroy_guard dguard2(new_begin, insert_pos);

			std::uninitialized_move_n(m_begin + offset, old_size - offset, insert_pos + 1);
	...
	reference emplace_back(Args&&... args)
	{
		...
		else
		{
			...
			std::construct_at(insert_pos, std::forward<Args>(args)...);
			destroy_guard dguard(insert_pos, insert_pos + 1);
	...
	void resize(size_type count)
	{
		...
		else if (count > old_size)
		{
			...
			else
			{
				...
				destroy_guard dguard(new_begin + old_size, new_begin + count);
...
private:
	...
	void construct_n(size_type count, Args&&... args)
	{
		...
		memory_guard mguard(new_begin, count);
	...
	void change_array(pointer new_begin, size_type new_size, size_type new_cap)
	{
		...
		{
			std::destroy(m_begin, m_end);
			detail::deallocate(m_begin, capacity());
		}
